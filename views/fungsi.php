...
function get_data_mhs($db_object, $nim){
...
function sudah_klasifikasi($db_object, $nim){
    $sql = "SELECT COUNT(*) FROM tb_data_mhs "
            . " WHERE nim = ".$nim;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row[0]>0;
...
function get_hasil_klasifikasi($db_object, $nim){
    $sql = "SELECT * FROM tb_data_spk WHERE nim = ".$nim;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row;
}
